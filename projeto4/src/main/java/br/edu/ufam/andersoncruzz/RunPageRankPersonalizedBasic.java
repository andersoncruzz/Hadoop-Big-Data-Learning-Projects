 ...
 */
public class RunPageRankPersonalizedBasic extends Configured implements Tool {
  private static final Logger LOG = Logger.getLogger(RunPageRankPersonalizedBasic.class);

  private static enum PageRank {
    nodes, edges, massMessages, massMessagesSaved, massMessagesReceived, missingStructure, source
  };
  ...
  private static class ReduceClass extends
    ...
    private float totalMass = (float) (Math.log(ALPHA) - Math.log(1));;
	  //private float totalMass = -93;
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
      ...
      //float mass = -93;
      float mass = (float) (Math.log(ALPHA) - Math.log(1));
  ...
  private static class MapPageRankMassDistributionClass extends
    ...
    public void setup(Context context) throws IOException {
      ...
      source = conf.getInt(SOURCE, 11044077);
      //source = vsource[0];
    }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
       ...
       * 
       */
     // if (node.getNodeId() == source || nid.get() == source) {
    	  float jump = (float) (Math.log(ALPHA) - Math.log(1));
      		//float jump = (float) 0.9;
    	  float link = (float) Math.log(1.0f - ALPHA)
      				+ sumLogProbs(p, (float) (Math.log(missingMass) - Math.log(1)));
  ...
  private static NumberFormat formatter = new DecimalFormat("0000");
	...
	public static void main(String[] args) throws Exception {
		ToolRunner.run(new RunPageRankPersonalizedBasic(), args);
	}

	public RunPageRankPersonalizedBasic() {}
  ...
  public int run(String[] args) throws Exception {
    ...
    options.addOption(OptionBuilder.withArgName("num").hasArg()
            .withDescription("source").create(SOURCE));
    ...
    if (!cmdline.hasOption(BASE) || !cmdline.hasOption(START) ||
      ...
      return -1;
	    ...
	    int source = Integer.parseInt(cmdline.getOptionValue(SOURCE));
    ...
    LOG.info(" - INMAPPER COMBINER: " + useRange);
  ...
  private void iteratePageRank(int i, int j, String basePath, int numNodes,
      boolean useCombiner, boolean useInMapperCombiner, int source) throws Exception {
    // Each iteration consists of two phases (two MapReduce jobs).

    // Job 1: distribute PageRank mass along outgoing edges.
    float mass = phase1(i, j, basePath, numNodes, useCombiner, useInMapperCombiner, source);

    // Find out how much PageRank mass got lost at the dangling nodes.
   
    //float missing = (float) (Math.log(ALPHA)) - (float) StrictMath.exp(mass);
    //float missing = (float) (Math.log(ALPHA) - Math.log(1));
    float missing = mass;
  ...
  private float phase1(int i, int j, String basePath, int numNodes,
    ...
    job.setJarByClass(RunPageRankPersonalizedBasic.class);
    ...
    job.getConfiguration().setInt(SOURCE, 11044077);
